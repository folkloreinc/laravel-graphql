...
class GraphQL {
    ...
    public function schema($schema = null)
    {
        ...
        {
            $query = $this->app->make($configQuery)->toType();
        }
        else
        {
            $queryFields = array_merge($configQuery, $this->queries);

            $query = $this->buildTypeFromFields($queryFields, [
                'name' => 'Query'
            ]);
        }
        ...
        {
            $mutation = $this->app->make($configMutation)->toType();
        }
        else
        {
            $mutationFields = array_merge($configMutation, $this->mutations);

            $mutation = $this->buildTypeFromFields($mutationFields, [
                'name' => 'Mutation'
            ]);
        }

        return new Schema(compact('query', 'mutation'));
    }
